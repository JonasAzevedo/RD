...
{$IFNDEF JPEGSO}
function malloc: Pointer; external 'libc.so.6' name 'malloc';
procedure free(P: Pointer); external 'libc.so.6' name 'free';
procedure memset(P: Pointer; B: Byte; count: Integer); external 'libc.so.6' name 'memset';
procedure memcpy(dest, source: Pointer; count: Integer);external 'libc.so.6' name 'memcpy';
...
function ferror(S: TStream): LongBool; cdecl;
begin
  Result := S = nil;
end;
...
function jpeg_start_decompress(var cinfo: jpeg_decompress_struct): Longbool; {$IFDEF LINUX} cdecl; {$ENDIF}
  external {$IFDEF JPEGSO} 'libjpeg.so' name 'jpeg_start_decompress' {$ENDIF};
...
procedure EmitMessage(cinfo: j_common_ptr; msg_level: Integer); {$IFDEF LINUX} cdecl; export; {$ENDIF}
begin
  //!!
end;

procedure OutputMessage(cinfo: j_common_ptr); {$IFDEF LINUX} cdecl; export; {$ENDIF}
begin
  //!!
end;

procedure FormatMessage(cinfo: j_common_ptr; buffer: PChar); {$IFDEF LINUX} cdecl; export; {$ENDIF}
begin
  //!!
end;

procedure ResetErrorMgr(cinfo: j_common_ptr); {$IFDEF LINUX} cdecl; export; {$ENDIF}
begin
...
procedure Init_Source(cinfo: j_decompress_ptr); cdecl; export;
begin
  my_source_ptr(cinfo.src).StartOfFile := True;
end;

function Fill_Input_Buffer(cinfo: j_decompress_ptr): LongBool; cdecl; export;
var
  nBytes: Integer;
  Src: my_source_ptr;
begin
...
procedure Skip_Input_Data(cinfo: j_decompress_ptr; num_bytes: Longint); cdecl; export;
var
  Src: my_source_ptr;
begin
  Src := my_source_ptr(cinfo.src);
...
function jpeg_resync_to_restart(cinfo: j_decompress_ptr; desired: Integer): LongBool; cdecl;
  external 'libjpeg.so' name 'jpeg_resync_to_restart';

procedure Term_Source(cinfo: j_decompress_ptr); cdecl; export;
begin
  { no work nessesary here }
end;

procedure JPEGStdioSrc(var cinfo: jpeg_decompress_struct; Stream: TStream);
var
  Src: my_source_ptr;
begin
...
procedure init_destination(cinfo: j_compress_ptr); cdecl; export;
var
...
function empty_output_buffer(cinfo: j_compress_ptr): LongBool; cdecl; export;
var
  Dest: my_destination_ptr;
begin
  Dest := my_destination_ptr(cinfo.dest);
...
procedure term_destination(cinfo: j_compress_ptr); cdecl; export;
var
  Dest: my_destination_ptr;
  DataCount: Integer;
begin
...
procedure JPEGStdioDest(var cinfo: jpeg_compress_struct; outfile: TStream);
var
  Dest: my_destination_ptr;
begin
