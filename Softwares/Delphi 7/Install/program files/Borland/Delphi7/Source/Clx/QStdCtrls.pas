...
type
  ...
  protected
    ...
    procedure GetSize(var AWidth, AHeight: Integer); virtual;
    procedure InitWidget; override;
  ...
  protected
    ...
    procedure Loaded; override;
    procedure ReturnPressed(var Suppress: Boolean); dynamic;
  ...
  private
    ...
    FMaxLength: Integer;
    FUndoLevels: Integer;
    FMouseUpAfterDrag: QMouseEventH;
    FMemStream: TMemoryStream;
    procedure CanUndoHook(p1: Boolean); cdecl;
    procedure CanRedoHook(p1: Boolean); cdecl;
    ...
    procedure SetUndoLevels(const Value: Integer);
    function GetCanRedo: Boolean;
    function GetCanUndo: Boolean;
  protected
    procedure Change; virtual;
    ...
    function GetCaretPos: TCaretPos; virtual;
    function GetSelLength: Integer; virtual;
    function GetSelStart: Integer; virtual;
    function GetSelText: WideString; virtual;
    function GetText: TCaption; override;
    ...
    property ShowHint;
    property TabStop default True;
    property UndoLevels: Integer read FUndoLevels write SetUndoLevels default 256;
  ...
  public
    ...
    procedure Redo;
    function Search(const Text: WideString; CaseSensitive, HighlightMatch, WholeWords,
      RegularExpr: Boolean; StartLine: Integer = 0; StartCol: Integer = 0): Boolean; virtual;
    procedure SelectAll;
    procedure Undo;
    procedure UnSelect;
    property CanRedo: Boolean read GetCanRedo;
  ...
  private
    ...
    procedure ItemMeasureHook(item: QClxListBoxItemH; var height, width: Integer); cdecl;
  ...
  protected
    ...
    function EditEventFilter(Sender: QObjectH; Event: QEventH): Boolean; virtual; cdecl;
    function ListEventFilter(Sender: QObjectH; Event: QEventH): Boolean; virtual; cdecl;
    function EventFilter(Sender: QObjectH; Event: QEventH): Boolean; override;
...
type
  ...
  private
    ...
    procedure ItemPaintHook(listBox: QListBoxH; p: QPainterH; Index: Integer;
      Rect: PRect; Flags: Integer; var Handled: Boolean); cdecl;
    procedure OnItemHook(item: QListBoxItemH); cdecl;
  ...
  protected
    ...
    procedure InternalSetItemData(Index: Integer; Data: Longint); virtual;
    procedure HookEvents; override;
    procedure Loaded; override;
...
uses RTLConsts, QConsts, QClipbrd;
...
type
  ...
  QClxListItem_measure_Event = procedure(item: QClxListBoxItemH; var height,
...
begin
  ...
  QListBox_setCurrentItem(ListBoxHandle, -1);
  FOwner.Invalidate;
end;
...
begin
  TempString := Strings[Index1];
  TempData := TCustomListBox(FOwner).InternalGetItemData(Index1);
  Strings[Index1] := Strings[Index2];
  TCustomListBox(FOwner).InternalSetItemData(Index1,
    TCustomListBox(FOwner).InternalGetItemData(Index2));
  Strings[Index2] := TempString;
  TCustomListBox(FOwner).InternalSetItemData(Index2, TempData);
end;
...
begin
  inherited Clear;
  QWidget_resize(ListBoxHandle, 0, 0);
end;
...
function TCustomGroupBox.GetClientRect: TRect;
begin
  Result := inherited GetClientRect;
  if Caption <> '' then
    Inc(Result.Top, Font.Height);
end;
...
begin
  inherited;
  InvalidateRect(Rect(0, 0, Width, Height), True);
end;

procedure TCustomGroupBox.PaletteChanged(Sender: TObject);
begin
...
procedure TCustomLabel.BorderStyleChanged;
begin
...
begin
  ...
  begin
    ...
    if Alignment = taRightJustify then
      ...
      NewLeft := NewLeft - (NewWidth - Width) div 2;
...
begin
  ...
  Hooks := QClxLineEdit_hook_create(Handle);
end;

procedure TCustomEdit.CutToClipboard;
begin
  QClxLineEdit_cut(Handle);
end;
...
begin
  Result := QAlignmentToTAlignment(QClxLineEdit_alignment(Handle));
end;

function TCustomEdit.GetBorderStyle: TControlBorderStyle;
begin
  if QClxLineEdit_frame(Handle) then
    Result := bsSingle
  else
    Result := bsNone;
end;
...
begin
  Result := TEchoMode(QClxLineEdit_echoMode(Handle));
end;

function TCustomEdit.GetModified: Boolean;
begin
  Result := QClxLineEdit_edited(Handle);
end;
...
begin
  QClxLineEdit_text(Handle, PWideString(@Result));
end;
...
begin
  ...
  begin
    ...
    QClxLineEdit_insert(Handle, PWideString(@WValue));
    QClxLineEdit_resetSelection(Handle);
  end;
end;
...
begin
  QClxLineEdit_selectAll(Handle);
end;

procedure TCustomEdit.SetAlignment(const Value: TAlignment);
begin
  if Value <> Alignment then QClxLineEdit_setAlignment(Handle, AlignmentFlags[Value]);
end;

procedure TCustomEdit.SetBorderStyle(const Value: TControlBorderStyle);
begin
  if BorderStyle <> Value then QClxLineEdit_setFrame(Handle, Boolean(Value));
end;

procedure TCustomEdit.SetEchoMode(const Value: TEchoMode);
begin
  if EchoMode <> Value then QClxLineEdit_setEchoMode(Handle, QLineEditEchoMode(Value));
end;
...
begin
  ...
  begin
    ...
    begin
      FMaxLength := -1;
      QClxLineEdit_setMaxLength(Handle, 32767);
    end
    else begin
      FMaxLength := Value;
      QClxLineEdit_setMaxLength(Handle, FMaxLength);
    end;
  end;
end;
...
begin
  if Modified <> Value then QClxLineEdit_setEdited(Handle, Value);
end;
...
begin
  ...
  begin
{$IFDEF LINUX}
    // Text width cannot exceed 32767 pixels due to a bug in QPainter::drawText()
    // which can potentially kill the X server.
    NewText := Value;
    FM := QFontMetrics_create(Font.Handle);
    try
      W := QFontMetrics_width(FM, @NewText, -1);
      if W >= 32767 then
      begin
        Len := Length(NewText) div 2;
        Last := Length(NewText);
        W := QFontMetrics_width(FM, @NewText, Len);
        while (W < 30000) or (W >= 32767) do
        begin
          Tmp := Len;
          if W < 30000 then
            Len := Len + (Last - Len) div 2
        ...
        SetLength(NewText, Len);
      end;
    finally
      QFontMetrics_destroy(FM);
    end;
    QClxLineEdit_setText(Handle, PWideString(@NewText));
{$ENDIF}
{$IFDEF MSWINDOWS}
    QClxLineEdit_setText(Handle, PWideString(@Value));
{$ENDIF}
    QClxLineEdit_resetSelection(Handle);
    TextChanged;
  end;
...
procedure TCustomEdit.DoStartDrag(var DragObject: TDragObject);
begin
  FDragging := True;
...
function TCustomEdit.EventFilter(Sender: QObjectH; Event: QEventH): Boolean;
  ...
  const
    ...
    idClear = 6;
    idSelectAll = 8;
    Captions: array[0..idSelectAll] of string = (
      SUndo, SRedo, SLine, SCut, SCopy, SPaste, SClear, SLine, SSelectAll);
  var
    LMenu: TPopupMenu;
    Item: TMenuItem;
    I: Integer;
    P: TPoint;
    IDs: array[idUndo..idSelectAll] of Integer;
    Selected: Integer;
  begin
    LMenu := TPopupMenu.Create(Self);
    try
      for I := idUndo to idSelectAll do
      begin
        Item := TMenuItem.Create(LMenu);
        Item.Caption := Captions[I];
        LMenu.Items.Add(Item);
        IDs[I] := Item.ID;
      end;

    LMenu.Items[idUndo].Enabled := CanUndo;
    LMenu.Items[idRedo].Enabled := CanRedo;
    LMenu.Items[idCut].Enabled := not ReadOnly and (SelLength > 0);
    LMenu.Items[idCopy].Enabled := SelLength > 0;
    LMenu.Items[idPaste].Enabled := not ReadOnly and (Length(Clipboard.AsText) > 0);
    LMenu.Items[idClear].Enabled := not ReadOnly and (Length(Text) > 0);
    LMenu.Items[idSelectAll].Enabled := (Length(Text) > 0)
      and (SelLength < Length(Text));

    P := QMouseEvent_globalPos(QMouseEventH(Event))^;
    Selected := QPopupMenu_exec(LMenu.Handle, @P, -1);
    for I := idUndo to idSelectAll do
      if Selected = IDs[I] then
      begin
        case I of
          idUndo: Undo;
          idRedo: Redo;
          idCut: CutToClipboard;
          idCopy: CopyToClipboard;
          idPaste: PasteFromClipboard;
          idClear: QClxLineEdit_clear(Handle);
          idSelectAll: SelectAll;
        end;
        Break;
      end;
    finally
  ...
  case QEvent_type(Event) of
    ...
    QEventType_mouseButtonRelease:
      begin
        ...
        if (QMouseEvent_button(QMouseEventH(Event)) = ButtonState_RightButton)
        and not Result then
        begin
          PopupLocalMenu;
          Result := True;
    ...
    QEventType_MouseMove:
      begin
        Result := inherited EventFilter(Sender, Event);
        // to prevent highlighting of text while a drag operation is pending.
        if FDragging or DragActivated then
        begin
          Result := True;
    ...
    QEventType_keyPress:
      begin
        ...
        begin
          ...
          if (ShiftState = [ssCtrl]) and not ReadOnly then
          begin
            case Key of
              Key_V:
                PasteFromClipboard;
          ...
          {$IFDEF MSWINDOWS}
          if (ShiftState = [ssShift]) and not ReadOnly then
          case Key of
            Key_Insert:
              begin
                PasteFromClipboard;
          ...
          if not Result and (CharCase <> ecNormal) then
            Result := EditCharCaseHandler(QKeyEventH(Event), CharCase, QLineEditH(Handle));
...
begin
  ...
  QClxLineEdit_textChanged_Event(Method) := TextChangedHook;
  QClxLineEdit_hook_hook_textChanged(QClxLineEdit_hookH(Hooks), Method);
end;
...
begin
  ...
  case Key of
    ...
    begin
      Suppress := False;
      ReturnPressed(Suppress);
      if Suppress then Key := 0;
    end;
  end;
...
function TCustomEdit.GetCursorPos: Integer;
begin
  Result := QClxLineEdit_cursorPosition(Handle);
end;

procedure TCustomEdit.SetCursorPos(const Value: Integer);
begin
  QClxLineEdit_setCursorPosition(Handle, Value);
end;

procedure TCustomEdit.Redo;
begin
  QClxLineEdit_redo(Handle);
end;

procedure TCustomEdit.ReturnPressed(var Suppress: Boolean);
begin
...
begin
  QClxLineEdit_undo(Handle);
end;

function TCustomEdit.GetCanRedo: Boolean;
begin
  Result := QClxLineEdit_canRedo(Handle);
end;

function TCustomEdit.GetCanUndo: Boolean;
begin
  Result := QClxLineEdit_canUndo(Handle);
end;


procedure TCustomEdit.DoBeep;
begin
  Beep;
end;
...
procedure TButtonControl.CreateWidget;
begin
  FHandle := QButton_create(ParentWidget, nil, WidgetFlags);
  Hooks := QButton_hook_create(FHandle);
end;
...
procedure TCustomComboBox.BeginAutoDrag;
var
  P: TPoint;
  R: TRect;
begin
  P := ScreenToClient(Mouse.CursorPos);
  //Contrary to the documentation, calling QStyle::comboButtonRect with the combobox's
  //bounds returns the area *excluding* the button
  QStyle_comboButtonRect(Application.Style.Handle, @R, 0, 0, Width, Height);
...
begin
  try
    ...
    begin
      ...
      begin
        //prevent firing of OnChange if an AutoCompleted match has been found.
        TextIndex := Items.IndexOf(Text^);
        if (FTempIndex = TextIndex) and (TextIndex > -1) then
          Exit
        else
          FTempIndex := TextIndex;
      end;
      Change;
    end;
  except
...
begin
  ...
  Hooks := QComboBox_hook_create(FHandle);
end;
...
begin
  case QEvent_Type(Event) of
    QEventType_MouseButtonPress:
      // Allow Qt to popup thier local menu only if we don't have a popup menu
      // assigned. It's not possible to retrieve enough information from QComboBox
      // to create our own local menu.
      if (Sender = EditHandle)
      and (QMouseEvent_button(QMouseEventH(Event)) = ButtonState_RightButton) then
      begin
        ...
        Result := Assigned(PopupMenu) and PopupMenu.AutoPopup;
        inherited EventFilter(Sender, Event);
      end
...
begin
  try
    ...
    if DroppedDown then
    begin
      // Work around Qt bug where QComboBox never considers the 0th item to
      // be selected when the list is down.
      FTempIndex := Index;
      Highlighted(Index);
    end;
  except
...
begin
  ...
  EditHandle;
  ListBoxHandle;
  FItemHeight := Canvas.TextHeight('Pq');
  ItemIndex := -1;
end;

procedure TCustomComboBox.InternalSetStyle;
var
  Method: TMethod;
begin
...
begin
  if FRecreateNeeded and not (csRecreating in ControlState) then
    RecreateWidget;
  inherited Loaded;
  if FRecreateNeeded and not (csRecreating in ControlState) then
  begin
...
begin
  ...
  begin
    if Value >= 0 then
      QComboBox_setMaxCount(Handle, Value-1)
    else
...
procedure TCustomComboBox.ItemMeasureHook(item: QClxListBoxItemH;
  var height, width: Integer);
begin
  if FStyle in [csOwnerDrawVariable, csOwnerDrawFixed] then
  try
    height := FItemHeight;
      if FStyle = csOwnerDrawVariable then
      MeasureItem(Self, item, height, width);
  except
    Application.HandleException(Self);
  end;
end;
...
procedure TCustomComboBox.MeasureItem(Control: TWinControl;
  item: QClxListBoxItemH; var Height, Width: Integer);
var
  Index: Integer;
begin
  if (Style = csOwnerDrawVariable) and Assigned(FOnMeasureItem) then
  begin
    Index := QListBox_index(ListBoxHandle, item);
...
begin
  if FStyle in [csOwnerDrawFixed, csOwnerDrawVariable] then
    ...
    // The choice of 6 in the statement below is based on the value in AdjustHeight
...
begin
  inherited Create(AOwner);
  ControlStyle := ControlStyle + [csCustomHints];
  FStyle := lbStandard;
...
begin
  FHandle := QListBox_create(ParentWidget, nil, WidgetFlags);
...
procedure TCustomListBox.DoStartDrag(var DragObject: TDragObject);
begin
  FDragging := True;
  inherited DoStartDrag(DragObject);
end;
...
begin
  ...
  if PtInRect(ClientRect, Pos) then
    Result := QListBox_index(Handle, QListBox_itemAt(Handle, @Pos));
...
begin
  ...
  if FRowState.RowLayout = llFixedNumber then
    ...
    RowLayout := FRowState.RowLayout;
  if FRowState.ColLayout = llFixedNumber then
    Columns := FRowState.Cols
  else
    ColumnLayout := FRowState.ColLayout;
  // Workaround crash in Qt because columns are not initialized for
  // certain states of RowLayout and ColumnLayout
  Columns;
end;
...
procedure TCustomListBox.ScrollBy(DeltaX, DeltaY: Integer);
begin
...
begin
  FRowState.ColLayout := Value;
  if (ColumnLayout <> Value) and not (csLoading in ComponentState) then
    QListBox_setColumnMode(Handle, QListBoxLayoutMode(Value));
end;

procedure TCustomListBox.SetColumns(const Value: Integer);
begin
  FRowState.Cols := Value;
  if (Columns <> Value) and (not (csLoading in ComponentState)
  and (ColumnLayout = llFixedNumber)) then
...
begin
  FRowState.RowLayout := Value;
  if (RowLayout <> Value) and not (csLoading in ComponentState) then
    QListBox_setRowMode(Handle, QListBoxLayoutMode(Value));
end;

procedure TCustomListBox.SetRows(const Value: Integer);
begin
  FRowState.Rows := Value;
  if (Rows <> Value) and (not (csLoading in ComponentState)
  and (RowLayout = llFixedNumber)) then
...
begin
  ...
  QListBox_onItem_Event(Method) := OnItemHook;
  QListBox_hook_hook_onItem(QListBox_hookH(Hooks), Method);
...
procedure TCustomListBox.ItemMeasureHook(item: QClxListBoxItemH;
  var height, width: Integer);
begin
  try
    height := FItemHeight;
...
begin
  try
    ...
    if not TCustomListBoxStrings(Items).FPutting and not FClicking and
      not FDeleting and not FSuppressOnClick then
...
begin
  ...
  case QEvent_type(Event) of
    ...
    QEventType_MouseMove:
      begin
        Result := inherited EventFilter(Sender, Event);
        // to prevent changing selected item while a drag operation is pending.
        if FDragging or DragActivated then
        begin
          Result := True;
          if not DragActivated then
...
function TCustomListBox.DoCustomHint(var HintInfo: THintInfo): Boolean;
var
  Item: Integer;
  BRect,
  TextRect: TRect;
begin
  Item := ItemAtPos(ScreenToClient(Mouse.CursorPos), True);
  Result := Item >= 0;
  if Result then
  begin
    TextRect := ItemRect(Item);
    BRect := TextRect;
    Canvas.TextExtent(Items[Item], BRect,
        Integer(AlignmentFlags_SingleLine));
    Result := (BRect.Right > ClientWidth) or (BRect.Left < 0);
    if Result then
      HintInfo.HintStr := Items[Item];
  end;
  Result := inherited DoCustomHint(HintInfo) or Result;
end;

procedure TCustomListBox.OnItemHook(item: QListBoxItemH);
begin
  try
    Application.CancelHint;
  except
...
procedure TCustomListBox.InternalSetItemData(Index, Data: Integer);
begin
...
procedure TMemoStrings.DefineProperties(Filer: TFiler);
...
begin
  ...
  CaretPos.Col := 0;
  Memo.SetCaretPos(CaretPos);
  //QMultiLineEdit has trouble painting after adding
  //strings with embedded or trailing LF's
  if (Pos(#10, S) > 0) and (UpdateCount = 0) then
    Memo.Repaint;
  Changed;
end;
...
procedure TMemoStrings.ReadData(Reader: TReader);
begin
...
procedure TMemoStrings.WriteData(Writer: TWriter);
var
  S: string;
  I: Integer;
begin
  ...
  begin
    S := Get(I);
    while not QOpenMultiLineEdit_isEndOfParagraph(Memo.Handle, I)
    and (I < Count) do
    begin
      Inc(I);
      S := S + Get(I);
...
begin
  ...
  // bail if there's no text, because QMultiLineEdit_setText will *always* insert
  // a linefeed.
  if Value = '' then Exit;
...
begin
  ...
  //QMultiLineEdit doesn't visually update the caret position
  //while updates are disabled.
  CaretPos := Memo.GetCaretPos;
  Memo.SetCaretPos(CaretPos);
  if not Updating then
    QTableView_repaint(Memo.Handle, True);
  if not Updating and FChanged then
  begin
    if (Memo.ScrollBars in [ssAutoVertical, ssAutoHorizontal, ssAutoBoth])
    and Memo.WordWrap then
      //force QMultiLineEdit to redo it's wrapping, since it doesn't when
      //auto scrollbars appear or disappear.
      Memo.ForceUpdate;
    Changed;
  end;
  FChanged := False;
...
procedure TCustomMemo.DoStartDrag(var DragObject: TDragObject);
begin
  FDragging := True;
  inherited DoStartDrag(DragObject);
end;

procedure TCustomMemo.DoEndDrag(Target: TObject; X, Y: Integer);
begin
  if FMouseUpAfterDrag <> nil then
  begin
    QApplication_sendEvent(Handle, FMouseUpAfterDrag);
    FMouseUpAfterDrag := nil;
  end;
end;

function TCustomMemo.EventFilter(Sender: QObjectH;
  Event: QEventH): Boolean;
  ...
  case QEvent_type(Event) of
    ...
    QEventType_MouseButtonRelease:
      begin
        ...
        // The synthesized MouseButtonRelease event required by DragStart
        // can't be processed while dragging because QMultiLineEdit will
        // deselect the text. Postpone until dragging is completed.
        if (FDragging or DragActivated) and (FMouseUpAfterDrag = nil) then
        begin
          Pt := Mouse.CursorPos;
          FMouseUpAfterDrag := QMouseEvent_create(QEventType_MouseButtonRelease, @Pt,
...
procedure TCustomMemo.CanRedoHook(p1: Boolean);
begin
  FCanRedo := p1;
end;

procedure TCustomMemo.CanUndoHook(p1: Boolean);
begin
  FCanUndo := p1;
end;

procedure TCustomMemo.Redo;
begin
...
procedure TCustomMemo.SetUndoLevels(const Value: Integer);
begin
  if (FUndoLevels <> Value) then
  begin
    FUndoLevels := Value;
    if HandleAllocated then
    begin
      QMultiLineEdit_setUndoEnabled(Handle, FUndoLevels <> 0);
      if FUndoLevels <> 0 then
        QMultiLineEdit_setUndoDepth(Handle, FUndoLevels)
      else begin
        FCanRedo := False;
        FCanUndo := False;
      end;
...
function TCustomMemo.GetCanRedo: Boolean;
begin
  Result := Modified and FCanRedo and (FUndoLevels <> 0);
end;

function TCustomMemo.GetCanUndo: Boolean;
begin
  Result := Modified and FCanUndo and (FUndoLevels <> 0);
end;
...
procedure TCustomMemo.Insert(const Text: WideString; Select: Boolean = False);
begin
  QOpenMultiLineEdit_insert(Handle, PWideString(@Text), Select);
end;
...
procedure TCustomMemo.Insert(const Text: WideString; Column, Row: Integer;
  Mark: Boolean = False);
begin
  QMultiLineEdit_insertAt(Handle, PWideString(@Text), Row, Column, Mark);
end;
...
function TCustomMemo.Search(const Text: WideString; CaseSensitive, HighlightMatch,
  WholeWords, RegularExpr: Boolean; StartLine: Integer = 0;
  StartCol: Integer = 0): Boolean; 
begin
  Result := QOpenMultiLineEdit_search(Handle, @Text, StartLine, StartCol,
      CaseSensitive, HighlightMatch, WholeWords, RegularExpr) = 0;
end;
...
procedure TCustomMemo.ForceUpdate;
var
  ResizeEvent: QResizeEventH;
  Size: TSize;
begin
  // This is the only programmatic way to get QMultiLineEdit to recalculate its
  // text area
  if HandleAllocated then
  begin
    Size.cx := Width;
...
begin
  ...
  QMultiLineEdit_undoAvailable_Event(Method) := CanUndoHook;
  QMultiLineEdit_hook_hook_undoAvailable(QMultiLineEdit_hookH(Hooks), Method);
  QMultiLineEdit_redoAvailable_Event(Method) := CanRedoHook;
  QMultiLineEdit_hook_hook_redoAvailable(QMultiLineEdit_hookH(Hooks), Method);
end;

procedure TCustomMemo.RestoreWidgetState;
begin
  inherited RestoreWidgetState;
...
procedure TCustomMemo.SaveWidgetState;
begin
  inherited SaveWidgetState;
  FMemStream := TMemoryStream.Create;
  FMemStream.WriteComponent(Self);
...
begin
  ...
  begin
    E := QEvent_type(Event);
    if (E = QEventType_FocusIn)
    and (QFocusEvent_reason <> QFocusEventReason_ActiveWindow)
    and (QFocusEvent_reason <> QFocusEventReason_Mouse)
    and (QFocusEvent_reason <> QFocusEventReason_Popup) then
      SetChecked(True);
