...
type
  ...
  // for .xfm compatibility only. Does nothing in CLX 2.0
  TOrientation = (orVertical, orHorizontal);

  TColumnLayout = (clAutoSize, clSpaceEqual);
  ...
  private
    ...
    procedure WriteOrientation(Writer: TWriter);
  protected
...
begin
  ...
  if Assigned(FOnProgress) then FOnProgress(Sender, Stage, PercentDone, RedrawNow, R, Msg, Continue);
end;
...
function TGroupButton.CanFocus: Boolean;
begin
  Result := not FRadioGroup.FSettingIndex;
end;
...
begin
  if not (csDesigning in ComponentState) and
    ...
    begin
      Result := not TCustomRadioGroup(Parent).CanModify or Checked;
...
function TGroupButton.NeedKey(Key: Integer; Shift: TShiftState;
  const KeyText: WideString): Boolean;
var
  Index: Integer;
  Button: TGroupButton;
begin
  Result := inherited NeedKey(Key, Shift, KeyText) and
    FRadioGroup.CanModify;
  if Result then
    begin
    Index := FRadioGroup.FButtons.IndexOf(Self);
    case Key of
      Key_Up, Key_Left:
        Dec(Index);
      Key_Down, Key_Right:
    ...
    if Index < 0 then
      Index := FRadioGroup.FButtons.Count - 1
    else
      if Index > FRadioGroup.FButtons.Count - 1 then
        Index := 0;
    Button := TGroupButton(FRadioGroup.FButtons[Index]);
...
procedure TCustomRadioGroup.ReadOrientation(Reader: TReader);
begin
  FOrientation := TOrientation(GetEnumValue(TypeInfo(TOrientation),
    Reader.ReadIdent));
end;

procedure TCustomRadioGroup.WriteOrientation(Writer: TWriter);
begin
  Writer.WriteIdent(GetEnumName(TypeInfo(TOrientation), Integer(FOrientation)));
end;

procedure TCustomRadioGroup.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('Orientation', ReadOrientation, WriteOrientation,
    Orientation <> orVertical); { do not localize }
end;
...
procedure TCustomRadioGroup.FontChanged;
  ...
  var
    ...
    Size: TSize;
  const
    AlignFlags: Integer = Integer(AlignmentFlags_AlignLeft) or
      Integer(AlignmentFlags_AlignVCenter) or
      Integer(AlignmentFlags_SingleLine) or
      Integer(AlignmentFlags_ShowPrefix);
    Gutter = 6; //from qradiobutton.cpp
    Margin = 2; //from qradiobutton.cpp
begin
    QStyle_exclusiveIndicatorSize(QApplication_style, @Size);
    ...
    try
      for I := 0 to FButtons.Count - 1 do
        ...
        begin
          Str := Caption;
          QFontMetrics_boundingRect(FM, @R, 0, 0, 10000, 10000,
            AlignFlags, @Str, -1, 0, nil, nil);
          Width := R.Right - R.Left + Size.cx + Gutter + Margin;
...
procedure TCustomRadioGroup.AlignControls(AControl: TControl;
  var Rect: TRect);
const
  Margin = 8;
var
  MaxW, ColSpace, RowSpace, Rows, X, Y, I, Extra, Col: Integer;
  ColLefts: array of Integer;
  R: TRect;
begin
  //change layout order to ensure rows & columns fill correctly.
  if FButtons.Count = 0 then Exit;
  Rows := FButtons.Count div FColumns;
  Extra := FButtons.Count mod FColumns;
  while (Extra > 0) and (Extra <= FColumns) do
  begin
    Inc(Rows);
    Dec(Extra, FColumns);
  end;
  ...
  R.Top := R.Top - (Font.Height) div 2;
  RowSpace := (R.Bottom - R.Top + Margin) div Rows;
  ColSpace := (R.Right - R.Left {- Margin * (FColumns + 1)}) div FColumns;
  SetLength(ColLefts, FColumns);
  MaxW := 0;
  for I := 0 to Length(ColLefts) - 1 do
    if FColumnLayout = clSpaceEqual then
      ColLefts[I] := ColSpace + Margin
    else begin
      for X := 0 to Rows - 1 do
      begin
        if X mod Rows = 0 then
          MaxW := 0;
        if I + X > FButtons.Count - 1 then
          Break;
        with TGroupButton(FButtons[I + X]) do
          if Width > MaxW then MaxW := Width;
      end;
      ColLefts[I] := MaxW + Margin;
    end;
  ...
  begin
    ...
    with TGroupButton(FButtons[I]) do
    begin
      if (I mod Rows = 0) and (I > 0) and (Col < FColumns - 1) then
      begin
        Y := R.Top;
        Inc(Col);
        Inc(X, ColLefts[Col-1]);
      end;
      Height := RowSpace;
      Left := X;
      Top := Y;
...
begin
  ...
  for I := 0 to FButtons.Count - 1 do
    with TGroupButton(FButtons[I]) do
      QOpenWidget_updateMask(QOpenWidgetH(TGroupButton(FButtons[I]).Handle));
end;
...
procedure TCustomRadioGroup.SetColumnLayout(const Value: TColumnLayout);
begin
...
begin
  ...
  begin
    ...
    try
      if Value >= 0 then
        TGroupButton(FButtons[Value]).Checked := True
      else
...
begin
  ...
  for I := 0 to FButtons.Count - 1 do
    ...
    begin
      ...
      QStyle_exclusiveIndicatorSize(QApplication_style, @IndSize);
      Width := Size.cx + IndSize.cx + Gutter + Margin;
    end;
...
procedure TCustomRadioGroup.InitWidget;
begin
