...
{ TMenuItem }
  ...
  private
    ...
    procedure HighlightedHook(ItemID: Integer); virtual; cdecl;
  ...
  private
    ...
    function GetHandle: QPopupMenuH;
    procedure HighlightedHook(ItemID: Integer); override; cdecl;
    procedure ShowHook; cdecl;
  protected
    function MenuDataHandle: QMenuDataH; override;
    procedure CreateWidget; override;
  ...
  try
    ...
    begin
      ...
      if (csDesigning in ComponentState) and
         (Item.GetParentMenu <> nil) and
         (Item.GetParentMenu.Owner <> nil) then
         if (Item.GetParentMenu.Owner is TWidgetControl) then
            QApplication_postevent((Item.GetParentMenu.Owner as TWidgetControl).Handle,
              QCustomEvent_create(QEventType_DesignMenuClick, Item))
         else
         if  ((Item.GetParentMenu.Owner.Owner <> nil) and
             (Item.GetParentMenu.Owner.Owner is TWidgetControl)) then
                         QApplication_postevent((Item.GetParentMenu.Owner.Owner as TWidgetControl).Handle,
              QCustomEvent_create(QEventType_DesignMenuClick, Item))
    end;
  except
...
begin
  ...
  begin
    ...
    begin
      ...
      FBitmap.Free;
      FBitmap := nil;
      if ((ImageIndex < 0) or ((GetParentMenu <> nil)
      and not Assigned(GetParentMenu.Images))) then
        ChangePixmap(nil);
    end;
  end;
end;
...
begin
  ...
  begin
    NeedRebuild := (FCaption = cLineCaption) or (Value = cLineCaption);
    FCaption := Value;
...
begin
  ...
  begin
    ...
    begin
      NewIconSet := QIconSet_create(Value, QIconSetSize_Small);
      try
        QMenuData_changeItem(Parent.MenuData, ID, NewIconSet, @FCaption);
      finally
        QIconSet_destroy(NewIconSet);
      end;
    end
...
procedure TMenu.BitmapChanged(Sender: TObject);
begin
  if (FHandle <> nil) then
...
begin
  ...
  FPalette.Widget := QWidgetH(FHandle);
  FColor := clButton;
end;
...
procedure TMenu.ColorChanged;
begin
  HandleNeeded;
  if Bitmap.Empty then
    Palette.BaseColor := FColor;
end;
...
var
  P: TPoint;
begin
  if Self is TPopupMenu then Exit;
  if Value <> nil then
  begin
...
begin
  ...
  QPopupMenu_popup(Handle, @FPopupPoint, -1);
  FCurrentItem := nil;
end;
