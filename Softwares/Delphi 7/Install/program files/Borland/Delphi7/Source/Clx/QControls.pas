...
{ TControl }
  ...
  TControlState = set of (csDragAllow, csLButtonDown, csClicked, csPalette,
  ...
  TControlStyle = set of (csAcceptsControls, csCaptureMouse,
    ...
    csMenuEvents, csNoFocus, csCustomHints);
  ...
  protected
    ...
    function DoCustomHint(var HintInfo: THintInfo): Boolean; dynamic;
...
function DragActivated: Boolean;
...
function TestWidgetFlags(Instance: TWidgetControl; const Flags: array of WidgetFlags): Boolean;
procedure SetWidgetFlags(Instance: TWidgetControl; const Flags: array of WidgetFlags);
procedure ClearWidgetFlags(Instance: TWidgetControl; const Flags: array of WidgetFlags);
function TestWidgetState(Instance: TWidgetControl; const States: array of WidgetState): Boolean;
procedure SetWidgetState(Instance: TWidgetControl; const States: array of WidgetState);
procedure ClearWidgetState(Instance: TWidgetControl; const States: array of WidgetState);
...
begin
  {$IFDEF MSWINDOWS}
  { The implementation of QApplication_widgetAt in qapplication_win.cpp is
    incorrect for windows when you pass true as the second parameter.
    It should call ScreenToClient, but does not. }
  Handle := nil;
  ...
  begin
    ...
    begin
      P := Pos;
      ScreenToClient(win, P); { This is missing in WidgetAt in QT }  
    	cwin := ChildWindowFromPoint(win,  p);
	    if (cwin <> 0) and (cwin <> win) then
	      Handle := QWidget_find( cwin )
      else
        Handle := w;
    end;
  end;
...
begin
  ...
  if AGeom.Right < AGeom.Left then
   Exit;
...
begin
  ...
  Result := QWidget_testWFlags(Instance.Handle, F);
end;
...
begin
  F := 0;
  for I := 0 to Ord(High(States)) do
   F := F or Cardinal(States[I]);
  Result := QWidget_testWState(Instance.Handle, F);
end;
...
begin
  ...
  for I := 0 to Ord(High(Flags)) do
   F := F or Cardinal(Flags[I]);
  QOpenWidget_SetWFlags(QOpenWidgetH(Instance.Handle), F);
end;
...
begin
  ...
  for I := 0 to Ord(High(States)) do
   F := F or Cardinal(States[I]);
  QOpenWidget_SetWState(QOpenWidgetH(Instance.Handle), F);
end;
...
begin
  ...
  for I := 0 to Ord(High(Flags)) do
   F := F or Cardinal(Flags[I]);
  QOpenWidget_clearWFlags(QOpenWidgetH(Instance.Handle), F);
end;
...
begin
  ...
  for I := 0 to Ord(High(States)) do
   F := F or Cardinal(States[I]);
  QOpenWidget_clearWState(QOpenWidgetH(Instance.Handle), F);
end;
...
begin
  try
    ...
    if csDragAllow in InitialDragControl.ControlState then
    begin
      Pt := Mouse.CursorPos;
...
begin
  { Notification }
  if (Parent <> nil) and Parent.HandleAllocated and not (csDesigning in ComponentState) then
    QWidget_setCursor(Parent.Handle, Screen.Cursors[FCursor]);
end;
...
begin
  if csCustomHints in ControlStyle then
...
function TControl.DoCustomHint(var HintInfo: THintInfo): Boolean;
begin
  Result := False;
end;
...
procedure TWidgetControl.BitmapChanged(Sender: TObject);
begin
  if HandleAllocated then
    Palette.SetWidgetBitmap(FBitmap);
end;
...
begin
  FStandardHint := not DoCustomHint(HintInfo);
  if FStandardHint then
    Result := not FHintShown
  else
    Result := True;
  if FStandardHint and not FHintShown then
    FHintShown := Result;
end;
...
function TWidgetControl.EventFilter(Sender: QObjectH; Event: QEventH): Boolean;
  ...
  begin
    ...
    case QEvent_type(Event) of
      ...
      begin
        Include(Control.FControlState, csDragAllow);
      ...
      begin
        try
          Exclude(Control.FControlState, csDragAllow);
          if MouseButton = mbLeft then
          ...
          if Control <> InitialDragControl then
            Result := Control.DoMouseUp(MouseButton, ShiftState, P.X, P.Y)
      ...
      begin
        if csDragAllow in Control.ControlState then
    ...
    case QEvent_type(Event) of
      QEventType_KeyPress:
        begin
          ...
          if Assigned(ParentForm) and (ParentForm <> Self)
          and ParentForm.KeyPreview then
...
begin
  ...
  case QEvent_type(Event) of
    ...
    QEventType_ParentPaletteChange:
      Result := not ParentColor;
    QEventType_CMPaletteReset:
      begin
        FColor := FPalette.Color;
        FBitmap.Free;
        FBitmap := nil;
      end;
    QEventType_Show,
...
begin
  ...
  begin
    NormalizeRect(Rect);
    DoesNotErase := TestWidgetFlags(Self, [WidgetFlags_WRepaintNoErase]);
    ...
    begin
      if not DoesNotErase then
        QOpenWidget_setWFlags(QOpenWidgetH(Handle),
          QOpenWidget_getWFlags(QOpenWidgetH(Handle)) or Integer(WidgetFlags_WRepaintNoErase));
      QWidget_update(ChildHandle, @Rect);
      if not DoesNotErase then
        QOpenWidget_setWFlags(QOpenWidgetH(Handle),
          QOpenWidget_getWFlags(QOpenWidgetH(Handle)) and not Integer(WidgetFlags_WRepaintNoErase));
    end;
  end;
end;
...
begin
  Result := Assigned(FBitmap) and not FBitmap.Empty and not QPixmap_isNull(FBitmap.Handle);
end;
...
{    case QEvent_type(Event) of
      ...
      begin
        ...
        Result := True;
        //Prevent Qt from passing unhandled key events up the widget's parent chain.
        //QKeyEvent_accept(QKeyEventH(Event));
      end;
    end;}
...
var
  FocusedControl: TWidgetControl;
begin
  FocusedControl := nil;
  Include(FControlState, csRecreating);
  try
    if (GetParentForm(Self) <> nil) then
      FocusedControl := GetParentForm(Self).ActiveControl;
    DestroyHandle;
    CreateHandle;
    ResubmitFlags;
    UpdateControlState;
...
begin
  if csDesigning in ComponentState then
    ...
    Result := not TestWidgetState(Self, [WidgetState_WState_ForceDisabled]);  
end;
...
begin
  ...
  if FMasked and HandleAllocated then
    QOpenWidget_updateMask(QOpenWidgetH(Handle));
  Invalidate;
...
begin
  ...
  if (Parent <> nil) and QObject_inherits(Parent.Handle, 'QScrollView') then
    QScrollView_addChild(QScrollViewH(Parent.Handle), Handle, P.X, P.Y);
...
{$IFDEF LINUX}
              or Integer(WidgetFlags_WX11BypassWM)
{$ENDIF}
...
begin
  ...
  try
    ...
    if UIEffect_UI_AnimateTooltip in Application.Effects then
      QClxDrawUtil_ScrollEffect(Handle, UIEffect_DownScroll, 75)
    else if UIEffect_UI_FadeTooltip in Application.Effects then
      QClxDrawUtil_FadeEffect(Handle, 200);
    QWidget_show(Handle);
...
procedure TFrameControl.BorderStyleChanged;
begin
  // Notification
end;
...
procedure TFrameControl.DoFlipChildren;
begin
//
end;
