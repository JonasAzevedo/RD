...
type
  ...
  protected
    ...
    procedure ShowEditorString(var S: WideString);
...
begin
  ...
  else if (Key = Key_Escape) and (goAlwaysShowEditor in Grid.Options) then
    Result := False
...
procedure TCustomGrid.ShowEditorString(var S: WideString);
var
  Handled : boolean;
begin
  ShowEditor;
  ...
  begin
    FInplaceEdit.KeyString(S, Handled);
    FInplaceEdit.Text := S;
    QClxLineEdit_Deselect(InplaceEditor.Handle);
  end;
end;
...
begin
  ...
  begin
    ...
    if Ch <> #0 then
      if Ch = #8 then
        QClxLineEdit_backspace(InplaceEditor.Handle)
      else begin
...
procedure TCustomGrid.KeyString(var S: WideString; var Handled: Boolean);
begin
...
begin
  ...
  if not CanEditShow then HideEdit;
end;
